...
public abstract class FGUtilCore {
	...
	protected SpleefArena plg;
	public String px = "";
	private String permprefix = SpleefArena.premPrefix + "fgutilcore.";
	...
	public FGUtilCore(SpleefArena plg, boolean vcheck, String lng, String devbukkitname, String px) {
		this.plg = plg;
	...
	public double getNewVersion(double currentVersion) {
		try {
			...
			if (firstNode.getNodeType() == 1) {
				...
				version_new_str = firstNodes.item(0).getNodeValue().replace("MagicWand v", "").trim();
				
				return Double.parseDouble(firstNodes.item(0).getNodeValue().replace("MagicWand v", "").trim());
			}
		} catch (Exception e) {
			plg.getServer().getConsoleSender().sendMessage("[SpleefArena] Failed to get updates");
		}
